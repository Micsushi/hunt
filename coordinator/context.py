...
def derive_manual_review_flags(job: Mapping[str, Any]) -> list[str]:
    return _dedupe(
        [flag for flag in derive_concern_flags(job) if flag in _MANUAL_REVIEW_SEED_FLAGS]
    )
...
def build_apply_context_payload(
    ...
    *,
    ...
    return {
        ...
        "selected_resume_version_id": _string_or_none(
            _mapping_get(job, "selected_resume_version_id")
        ),
        ...
        "job_description_path": _string_or_none(
            _mapping_get(job, "latest_resume_job_description_path")
        ),
...
def build_c3_apply_payload(
    ...
    *,
    ...
    payload = {
        ...
        "selectedResumeVersionId": _string_or_none(_mapping_get(job, "selected_resume_version_id"))
        or "",
        "selectedResumePath": selected_resume_path,
        "selectedResumeTexPath": _string_or_none(_mapping_get(job, "selected_resume_tex_path"))
        or "",
    ...
    if embed_resume_data:
        ...
        payload["selectedResumeName"] = (
            Path(selected_resume_path).name if selected_resume_path else ""
        )
