...
class OrchestrationService:
    """DB-backed C4 (Coordinator) orchestration flow."""
        ...
        try:
            ...
            if "no such table" in str(exc).lower():
                raise OrchestrationError(
                    "The Hunt jobs table is missing. Initialize C1 (Hunter) data before running C4."
                ) from exc
            raise
    ...
    def _append_event(
        ...
        *,
        ...
        payload_json = (
            json.dumps(payload, indent=2, sort_keys=True) if payload is not None else None
        )
        cursor = conn.execute(
            """
        ...
        row = conn.execute(
            "SELECT * FROM orchestration_events WHERE id = ?", (cursor.lastrowid,)
        ).fetchone()
    ...
    def _decision_from_row(self, conn: sqlite3.Connection, row: sqlite3.Row) -> ReadyJobDecision:
        ...
        if not _truthy(row["auto_apply_eligible"]):
            return ReadyJobDecision(
                job_id=job_id,
                ready=False,
                reason="not_auto_apply_eligible",
                source=row["source"],
                title=row["title"],
                company=row["company"],
                apply_url=row["apply_url"],
        ...
        if (
            not _truthy(row["selected_resume_ready_for_c3"])
            or not _text(row["selected_resume_version_id"])
            or not _text(row["selected_resume_pdf_path"])
        ):
            return ReadyJobDecision(
                job_id=job_id,
                ready=False,
                reason="waiting_on_resume",
                source=row["source"],
                title=row["title"],
                company=row["company"],
                apply_url=row["apply_url"],
        ...
        return ReadyJobDecision(
            ...
            title=row["title"],
            company=row["company"],
            apply_url=_text(row["apply_url"]) or None,
            ats_type=_text(row["ats_type"]) or "unknown",
            selected_resume_version_id=_text(row["selected_resume_version_id"]) or None,
            selected_resume_pdf_path=_text(row["selected_resume_pdf_path"]) or None,
            flags=derive_concern_flags(row),
        )
    ...
    def list_ready_decisions(
        self, *, limit: int | None = 50, reason: str | None = None, only_ready: bool = False
    ) -> list[ReadyJobDecision]:
        with self._connect() as conn:
        ...
        with self._connect() as conn:
            ...
            return {
                ...
                "global_hold": {
                    ...
                    "reasons": _dedupe(
                        [_text(row["manual_review_reason"]) for row in global_hold_rows]
                    ),
                },
            }
    ...
    def request_fill(self, run_id: str) -> dict[str, Any]:
        with self._connect() as conn:
            ...
            if run.status != "apply_prepared":
                raise OrchestrationError(
                    f"Run {run_id} must be in apply_prepared before fill can be requested."
                )
    ...
    def _derive_review_flags(
        ...
        if _normalize_list(fill_result.get("lowConfidenceAnswers")) or _normalize_list(
            fill_result.get("low_confidence_answers")
        ):
            flags.append("low_confidence_answers")
        if _normalize_list(fill_result.get("missingRequiredFields")) or _normalize_list(
            fill_result.get("missing_required_fields")
        ):
            flags.append("missing_required_fields")
        if _truthy(fill_result.get("unexpectedMultiPageFlow")) or _truthy(
            fill_result.get("unexpected_multi_page_flow")
        ):
        ...
        if _truthy(fill_result.get("unsupportedStep")) or _truthy(
            fill_result.get("unsupported_step")
        ):
    ...
    def _browser_summary(
        self, *, run: OrchestrationRun, fill_result: dict[str, Any], review_flags: list[str]
    ) -> dict[str, Any]:
        return {
            "run_id": run.run_id,
            "job_id": run.job_id,
            ...
            "generated_answers_used": _truthy(
                fill_result.get("generatedAnswersUsed") or fill_result.get("generated_answers_used")
            ),
            "low_confidence_answers": _normalize_list(
                fill_result.get("lowConfidenceAnswers") or fill_result.get("low_confidence_answers")
            ),
            "missing_required_fields": _normalize_list(
                fill_result.get("missingRequiredFields")
                or fill_result.get("missing_required_fields")
            ),
        ...
        *,
        ...
        with self._connect() as conn:
            ...
            approval_path = self._write_json_artifact(
                ...
                / "approvals"
                / str(run.job_id)
                / f"{now.replace(':', '-').replace('+00:00', 'Z')}_{approval_id}.json",
                {
    ...
    def get_submit_approval(self, approval_id: str) -> SubmitApproval | None:
        with self._connect() as conn:
            row = conn.execute(
                "SELECT * FROM submit_approvals WHERE id = ?", (approval_id,)
            ).fetchone()
            return SubmitApproval.from_row(row) if row else None

    def mark_submitted(
        self, run_id: str, *, summary_json_path: str | Path | None = None
    ) -> dict[str, Any]:
        submitted_summary = {}
    ...
    def run_job(
        ...
        *,
        ...
        if prepare_only:
            ...
            if run is None:
                raise OrchestrationError(
                    f"Run {context.run_id} could not be loaded after apply-prep."
                )
            return {
                "decision": "prepared",
                "run": run.to_dict(),
