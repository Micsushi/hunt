...
def analyze_security_challenge(page):
    ...
    is_security_challenge = (
        (
            ...
            and (
                "text" in categories
                or "title" in categories
                or "heading" in categories
                or "url" in categories
            )
        )
...
def click_locator_and_capture_url(page, locator, *, timeout_ms, fast_ui=False):
    ...
    if (
        current_page_url
        and current_page_url != before_url
        and not looks_like_linkedin_url(current_page_url)
    ):
        try:
...
def capture_external_apply_url(
    page, locator, *, timeout_ms, existing_apply_url=None, fast_ui=False
):
    ...
    if continue_button:
        ...
        apply_url = click_locator_and_capture_url(
            page, continue_button, timeout_ms=timeout_ms, fast_ui=fast_ui
        )
        if apply_url:
            return apply_url
...
def detect_apply_result(page, *, existing_apply_url=None, timeout_ms=45000, fast_ui=False):
    ...
    if external_apply_button:
        ...
        print(
            "[enrich] Found an external-looking apply action but could not resolve a destination URL."
        )
...
def extract_external_description(context, apply_url, *, timeout_ms, fast_ui=False):
    ...
    try:
        ...
        try:
            ...
            print(
                "[enrich] No dedicated external description block found. Falling back to broad visible page text."
            )
            ...
            if generic_text:
                ...
                print(
                    f"[enrich] Using broad external page text fallback ({len(generic_text)} chars)."
                )
                return generic_text
            raise
    finally:
    ...
    try:
        ...
        if apply_result["apply_type"] == "easy_apply":
            ...
            try:
                ...
                description = extract_generic_page_text(
                    page, min_chars=150
                ) or normalize_optional_str(job.get("description"))
        ...
        try:
            ...
            if (
                ...
                print(
                    "[enrich] LinkedIn page lacked a usable description. Falling back to external application page."
                )
                try:
...
def enrich_claimed_linkedin_job(
    job,
    *,
...
def process_claimed_job(
    ...
    *,
    ...
    print(
        f"[enrich] Claimed LinkedIn job id={claimed_job['id']} company={claimed_job.get('company')} title={claimed_job.get('title')}"
    )
    try:
        ...
        if error_code == "auth_expired":
            ...
            if not _auth_recovery_attempted:
                ...
                if relogin_result.get("recovered"):
                    print(
                        "[enrich] Auth recovered from stored session or credentials. Retrying the same job once."
                    )
        ...
        if context is not None and error_code in BLOCKED_ERROR_CODES.union(
            {
        ...
        with open_linkedin_context(
            ...
            for index in range(limit):
                ...
                if result["status"] == "auth_paused":
                    print(
                        f"[batch] Stopping early because LinkedIn auth needs refresh: {result['error_code']}"
                    )
                    break
                ...
                if result["status"] == "failed":
                    ...
                    if ui_verify_blocked and is_ui_verifiable_error_code(error_code):
                        ...
                        print(
                            f"[batch] Queued blocked job id={result['job_id']} for interactive verification after the first pass."
                        )
                        continue

                    if should_stop_batch_after_failure(
                        error_code, ui_verify_blocked=ui_verify_blocked
                    ):
        ...
        try:
            with open_linkedin_context(
                ...
                for index, job_id in enumerate(blocked_job_ids, start=1):
                    ...
                    if not claimed_job:
                        print(
                            f"[batch-ui] Could not reclaim blocked LinkedIn job id={job_id} for UI verification."
                        )
                        result = {
                            "status": "failed",
                            "job_id": job_id,
                    ...
                    if result["status"] == "auth_paused":
                        print(
                            f"[batch-ui] Stopping early because LinkedIn auth needs refresh: {result['error_code']}"
                        )
                        break

                    if result["status"] == "failed" and should_stop_batch_after_failure(
                        result.get("error_code"), ui_verify_blocked=False
                    ):
                        print(
                            f"[batch-ui] Stopping early because of blocking error: {result['error_code']}"
                        )
                        break
        except LinkedInSessionError as exc:
            error_message = format_error_message(exc)
