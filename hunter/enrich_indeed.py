...
from hunter.browser_runtime import (  # noqa: E402
    BrowserRuntimeError,
...
DEFAULT_BROWSER_CHANNEL = (
    os.getenv("INDEED_BROWSER_CHANNEL") or os.getenv("LINKEDIN_BROWSER_CHANNEL") or None
)
USER_AGENT = (
...
APPLY_TEXT_RE = re.compile(
    r"\b(apply|continue application|apply now|apply on company site)\b", re.IGNORECASE
)
...
def enrich_indeed_job_in_context(context, job, *, timeout_ms=45000):
    ...
    try:
        ...
        if _detect_job_removed(body_text):
            raise IndeedEnrichmentError(
                "job_removed", "Indeed indicates the job is no longer available."
            )
        if any(pattern in body_text.lower() for pattern in RATE_LIMIT_PATTERNS):
            raise IndeedEnrichmentError(
                "rate_limited", "Indeed appears to be rate-limiting or challenging requests."
            )

        description = _extract_description_from_browser(page, job)
        apply_url = _detect_apply_url_from_browser(
            page, page.url, existing_apply_url=job.get("apply_url")
        )
...
def enrich_indeed_job(job, *, timeout_ms=45000):
    ...
    try:
        ...
        raise IndeedEnrichmentError(
            "unexpected_error", f"Could not load the Indeed job page: {exc}"
        ) from exc
    ...
    if _detect_rate_limited(response, body_text):
        raise IndeedEnrichmentError(
            ...
            partial_result=capture_text_artifacts(
                ...
                metadata={
                    "error_message": "Indeed appears to be rate-limiting or challenging requests."
                },
            ),
        )
    if _detect_job_removed(body_text):
    ...
    try:
        ...
        apply_url = _extract_apply_url(
            session, soup, response.url, existing_apply_url=job.get("apply_url")
        )
        apply_host = get_apply_host(apply_url)
        ats_type = detect_ats_type(apply_url)
        apply_type = "external_apply" if apply_url else "unknown"
...
def process_batch(
    *, limit, timeout_ms=45000, browser_channel=None, ui_verify_blocked=False, return_summary=False
):
    started_at = time.monotonic()
    results = []
    ...
    for index in range(limit):
        ...
        if result["status"] == "failed":
            ...
            if ui_verify_blocked and error_code in UI_VERIFIABLE_ERROR_CODES:
                ...
                print(
                    f"[indeed-batch] Queued blocked job id={result['job_id']} for browser verification after the first pass."
                )
                continue
        ...
        with open_browser_context(
            ...
            for index, job_id in enumerate(blocked_job_ids, start=1):
                ...
                if result["status"] == "failed" and _should_stop_batch_after_failure(
                    ...
                    print(
                        f"[indeed-batch-ui] Stopping early because of blocking error: {result['error_code']}"
                    )
                    break
...
def process_one_job(
    job_id=None, *, timeout_ms=45000, force=False, browser_channel=None, ui_verify=False
):
