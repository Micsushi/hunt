...
- After a Review result, close only your assigned p Chrome lane after capturing
  final UI, proof, console, and audit artifacts.
...
12. After your report and artifacts are complete, close only Review lanes.
    Preserve hard-failure and site/posting-state lanes unless explicitly told
    cleanup is allowed.
...
9. Recommend generalized fixes that keep the run moving: better question
...
10. If a wrong answer caused the failure by opening unsupported required
...
11. Do not close the lane after a hard failure or site/posting stop unless the
