...
| **C3** | **Executioner** | Planned v3 browser application journey. v2 was removed from `main`; v3 is not implemented. | **`executioner/`** | none |
| **C4** | **Coordinator** | Paused orchestration experiment retained for reference. | **`coordinator/`** | none |
...
- Paused C4 helpers remain in `coordinator/` and `scripts/hunterctl.py`, but operator calls stop without starting C4.
...
| Module | Contents |
|--------|----------|
| `shared/notifications.py` | Canonical Discord webhook util - `send_discord_webhook_message()`, `get_discord_webhook_url()`. Used by C1; C2 can import when needed. |
...
- Status: paused. Do not deploy or run.
