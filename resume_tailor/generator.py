...
def _merge_source_entries(candidate_entries: list[dict], library_entries: list[dict], *, project: bool = False) -> list[dict]:
    ...
    for entry in candidate_entries:
        ...
        if target is not entry:
            target.setdefault("bullet_candidates", []).extend(copy.deepcopy(entry.get("bullet_candidates", [])))
            target.setdefault("immutable_facts", []).extend(copy.deepcopy(entry.get("immutable_facts", [])))
            for field_name in (
                "company",
                "title",
                "location",
            ...
            for field_name in (
                ...
                if field_name in entry:
                    target[field_name] = list(dict.fromkeys((target.get(field_name, []) or []) + (entry.get(field_name, []) or [])))
    ...
    for entry in library_entries:
        ...
        target.setdefault("bullet_candidates", []).extend(copy.deepcopy(entry.get("bullet_candidates", [])))
        target["role_family_tags"] = list(dict.fromkeys((target.get("role_family_tags", []) or []) + (entry.get("role_family_tags", []) or [])))
        target["technology_tags"] = list(dict.fromkeys((target.get("technology_tags", []) or []) + (entry.get("technology_tags", []) or [])))
        target["job_level_tags"] = list(dict.fromkeys((target.get("job_level_tags", []) or []) + (entry.get("job_level_tags", []) or [])))
...
def _entry_matches_resume_entry(entry: ExperienceEntry | ProjectEntry, source_entry: dict, *, project: bool = False) -> bool:
    source_ids = {
    ...
    source_ids.add(_slugify(" ".join(part for part in (source_title, source_company, source_location) if part)))
...
def _build_resume_like_experience(source_entry: dict) -> ExperienceEntry | None:
    ...
    bullets = [candidate.get("text", "").strip() for candidate in source_entry.get("bullet_candidates", []) if candidate.get("text")]
    if not bullets:
        return None
            ...
            candidates.append(
                {
                    ...
                    "source_fact_id": (
                        (bullet.get("supported_fact_ids") or [bullet.get("bullet_id") or _stable_source_id(f"src_{entry.entry_id}", text)])[0]
                    ),
                    "mode": "reuse",
                    "score": _score_text(
                        " ".join(
...
def _augment_skill_bucket(
    ...
    *,
    ...
    for skill in source_skills:
        ...
        if not name:
            continue
        if any(keyword.lower() in name.lower() or name.lower() in keyword.lower() for keyword in must_haves):
    ...
    for value in existing_values:
        if any(keyword.lower() in value.lower() or value.lower() in keyword.lower() for keyword in must_haves):
...
def generate_tailored_resume(
    ...
    *,
    ...
    for record in ranked_experience:
        ...
        chosen_bullets = _select_bullets(record["bullet_candidates"], max_count=max_count, min_count=1)
        ...
        structured_experience = [
            {
                ...
                "bullet_plan": [
                    {
                        "source_fact_id": _stable_source_id(f"og_{fallback_entry.entry_id}", bullet),
                        "mode": "rewrite",
                        "text": _rewrite_bullet(bullet, must_haves[:2]),
                    }
                    for bullet in fallback_entry.bullets[: max(1, min(2, len(fallback_entry.bullets)))]
                ],
            }
        ]
        selected_experience[0].bullets = [item["text"] for item in structured_experience[0]["bullet_plan"]]
    ...
    project_budget = max(0, DEFAULT_MAX_TOTAL_BULLETS - sum(len(entry.bullets) for entry in doc.experience))
