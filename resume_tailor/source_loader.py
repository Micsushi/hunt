    ...
    for raw_line in lines:
        ...
        if current_skill_bucket:
            ...
            if stripped.startswith("- Name:"):
                ...
                current_skill["related_role_families"] = _split_tags(stripped.split(":", 1)[1].strip())
            continue
        ...
        if stripped.startswith("- Entry ID:"):
            ...
            if current_fact["fact_id"]:
                target = "project_fact_ids" if current_entry_kind == "project" else "experience_fact_ids"
                profile[target].append(current_fact["fact_id"])
        elif current_subsection == "Immutable facts" and current_fact and stripped.startswith("- Text:"):
        ...
        elif current_subsection == "Bullet candidates" and current_bullet and stripped.startswith("- Text:"):
            current_bullet["text"] = stripped.split(":", 1)[1].strip()
        elif current_subsection == "Bullet candidates" and current_bullet and stripped.startswith("- Supported by fact IDs:"):
            current_bullet["supported_fact_ids"] = _split_tags(stripped.split(":", 1)[1].strip())
        elif current_subsection == "Bullet candidates" and current_bullet and stripped.startswith("- Relevance tags:"):
...
def load_bullet_library(path: str | Path) -> dict:
    ...
    for raw_line in lines:
        ...
        if stripped.startswith("- Source entry ID:"):
            ...
            current_bullet["supported_fact_ids"] = _split_tags(stripped.split(":", 1)[1].strip())
        elif current_subsection == "Bullet candidates" and current_bullet and stripped.startswith("- Evidence notes:"):
            current_bullet["evidence_notes"] = stripped.split(":", 1)[1].strip()
        elif current_subsection == "Bullet candidates" and current_bullet and stripped.startswith("- Metrics:"):
