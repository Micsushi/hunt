...
from .pipeline import generate_resume_for_ad_hoc, generate_resume_for_job, generate_resumes_for_ready_jobs
...
def cmd_generate_ad_hoc(args: argparse.Namespace) -> int:
    result = generate_resume_for_ad_hoc(
        ...
        description=Path(args.jd_file).read_text(encoding="utf-8") if args.jd_file else args.description,
        label=args.label,
        resume_path=args.resume,
    )
    print(json.dumps(result, indent=2))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Component 2 Stage 0/1 resume tools.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    parse_cmd = subparsers.add_parser("parse-resume", help="Parse the OG resume into structured JSON.")
    ...
    init_cmd = subparsers.add_parser("init-db", help="Add Component 2 tables/columns to the Hunt DB.")
    init_cmd.add_argument("--db", default=str(get_db_path()))
    init_cmd.set_defaults(func=cmd_init_db)

    generate_job = subparsers.add_parser("generate-job", help="Generate a resume attempt for one Hunt job.")
    ...
    generate_ad_hoc = subparsers.add_parser("generate-ad-hoc", help="Generate a resume attempt from manual JD input.")
