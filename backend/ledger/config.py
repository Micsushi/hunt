        ...
        structure.write_text(
            "\n".join(
                [
                    ...
                    "",
                    "Root files:",
                    "- `LEDGER_STRUCTURE.md`: traversal guide for humans and agents.",
                    "- `schema.json`: event fields and source-of-truth declaration.",
                    "- `index.json`: rebuildable lightweight index metadata.",
                    "- `active.json`: active agent, lane, and session manifest pointers.",
                    "",
                    "C3 paths:",
                    "- Agents: `c3/agents/<YYYY-MM-DD>/<agent_id>/manifest.json` and `agent.jsonl`.",
                    "- Lanes: `c3/lanes/<YYYY-MM-DD>/<lane_id>/manifest.json` and `lane.jsonl`.",
                    ...
                    "",
                    "Common lookups:",
                    "- By agent: read `active.json.active_agents[agent_id].log_path`, or call `GET /api/ledger/agents/{agent_id}` / MCP `hunt_ledger_get_agent_log`.",
                    "- By session: read `active.json.active_sessions[session_id].log_path`, or call `GET /api/ledger/sessions/{session_id}` / MCP `hunt_ledger_get_session_log`.",
                    "- By command: call `GET /api/ledger/commands/{command_id}/timeline` / MCP `hunt_ledger_get_command_timeline`; this dedupes events across active logs.",
                    "",
                ]
            ),
            encoding="utf-8",
