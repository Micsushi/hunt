...
def _orchestration_runtime_root() -> Path:
    configured = os.getenv("HUNT_ORCHESTRATION_ROOT")
    if configured:
        return Path(configured).expanduser().resolve()
    if os.name == "nt":
        return (REPO_ROOT / ".runtime" / "orchestration").resolve()
    return Path("/home/michael/data/hunt/orchestration")
...
def _get_job_context(job_id: int) -> dict[str, Any] | None:
    scraper_db.init_db(maintenance=False)
    return scraper_db.get_apply_context_for_job(job_id)
...
def get_ready_decision(job_id: int) -> dict[str, Any]:
    ...
    reason = "ready"
    checks = (
        ("enrichment_status", context.get("enrichment_status") in {"done", "done_verified"}, "enrichment_incomplete"),
        ("apply_type", context.get("apply_type") == "external_apply", "not_external_apply"),
        ("auto_apply_eligible", _normalize_bool(context.get("auto_apply_eligible")), "auto_apply_ineligible"),
        ("priority", int(context.get("priority") or 0) == 0, "manual_only_priority"),
        ("selected_resume_ready_for_c3", _normalize_bool(context.get("selected_resume_ready_for_c3")), "resume_not_ready_for_c3"),
        ("selected_resume_pdf_path", bool(context.get("selected_resume_pdf_path")), "missing_selected_resume_pdf"),
...
def build_apply_prep_payload(
    ...
    *,
    ...
    payload = {
        ...
        "selectedResumeSummary": _build_resume_summary(context.get("selected_resume_tex_path") or ""),
    ...
    final_output_path = Path(output_path) if output_path else _next_apply_context_path(int(context["job_id"]))
