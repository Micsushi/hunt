...
export function CoordinatorPage() {
  ...
  const approveRun = useMutation({
    mutationFn: ({ runId, decision }: { runId: string; decision: 'approve' | 'deny' }) =>
      approveC4Run(runId, decision, reason),
    onSuccess: (res) => {
      setLastResult(res)
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={`${styles.panel} ${styles.panelStrong}`}>
        ...
        {lastResult ? (
          <pre className={styles.pre} style={{ marginTop: 12 }}>
        ...
        <div className={styles.tableWrap}>
          <table className={styles.table}>
            ...
            <tbody>
              {runRows.length === 0 ? (
                <tr>
                  <td colSpan={5} className={styles.meta} style={{ padding: '12px 10px' }}>
                    No runs yet.
                  </td>
                </tr>
              ) : (
                  ...
                  return (
                    <tr key={runId}>
                      ...
                      <td className={styles.mono}>
                        {timeAgo(String(run.updated_at ?? run.created_at ?? ''))}
                      </td>
                      <td>
                        <div className={styles.actions}>
                          <button
                            className={styles.btn}
                            disabled={!runId || approveRun.isPending}
                            onClick={() => approveRun.mutate({ runId, decision: 'approve' })}
                          >
                            Approve
                          </button>
                          <button
                            ...
                            disabled={!runId || approveRun.isPending}
                            onClick={() => approveRun.mutate({ runId, decision: 'deny' })}
                          >
                            Deny
                          </button>
                        </div>
