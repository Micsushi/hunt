# C3 v3 owner-test backlog

These Tier 4 checks remain unchecked until the owner tests an explicitly
published Tier 3 version. Never click final Workday Submit.

- [ ] Confirm the installed package exposes only the four documented MCP methods.
- [ ] Confirm the frozen report shows the exact accepted impact SHA, `S3-F2-T13` prerequisite, and zero activated F3 tasks/evidence.
- [ ] Confirm all four accepted fixtures run before the 40-slot offline reconciliation.
- [ ] Confirm `WD40-009` and `WD40-021` remain honest manifest-only slots with no invented variant or browser result.
...
A claimed-requirement failure reopens Tier 2.
