...
test("production helper pins PKCE loopback Gmail readonly profile equality and DPAPI", async () => {
  ...
  assert.match(
    source,
    /IntegerField\(response, "refresh_token_expires_in", 1, 31536000\)/u,
  );
  const tokenShape = /public sealed class HuntGmailToken[\s\S]*?public void Clear/u
    .exec(source)?.[0] ?? "";
  assert.doesNotMatch(tokenShape, /refresh_token_expires_in|RefreshTokenExpires/u);
  ...
  try {
    ...
    const result = spawnSync(
      ...
      [
        "-NoLogo", "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass",
        "-Command",
        `$source=$env:HUNT_TEST_SOURCE; Add-Type -Path $source -ReferencedAssemblies 'System.Security.dll','System.Web.dll','System.Web.Extensions.dll'; $flags=[Reflection.BindingFlags]'NonPublic,Static'; $method=[HuntInteractiveGmailOAuthSealer].GetMethod('ParseTokenResponse',$flags); $exact=[HuntInteractiveGmailOAuthSealer].GetMethod('ExactObject',$flags); if($null -eq $method -or $null -eq $exact) { exit 61 }; $at=[DateTimeOffset]::Parse('2026-08-01T12:00:00.000Z'); function Parse($json,$required) { $value=$exact.Invoke($null,@($json)); return $method.Invoke($null,@($value,$required,$at)) }; $interactive='{"access_token":"access-value","expires_in":3600,"refresh_token":"refresh-value","refresh_token_expires_in":604800,"scope":"https://www.googleapis.com/auth/gmail.readonly","token_type":"Bearer"}'; $refresh='{"access_token":"access-value-2","expires_in":3600,"refresh_token_expires_in":604799,"scope":"https://www.googleapis.com/auth/gmail.readonly","token_type":"Bearer"}'; $rotated='{"access_token":"access-value-3","expires_in":3600,"refresh_token":"refresh-value-2","refresh_token_expires_in":31536000,"scope":"https://www.googleapis.com/auth/gmail.readonly","token_type":"Bearer"}'; try { $one=Parse $interactive $true; $two=Parse $refresh $false; $three=Parse $rotated $false; if($one.RefreshValue.Length -lt 1 -or $null -ne $two.RefreshValue -or $three.RefreshValue.Length -lt 1) { exit 62 }; $one.Clear(); $two.Clear(); $three.Clear() } catch { exit 63 }; foreach($invalid in @('{"access_token":"a","expires_in":3600,"scope":"https://www.googleapis.com/auth/gmail.readonly","token_type":"Bearer"}','{"access_token":"a","expires_in":3600,"refresh_token":"r","refresh_token_expires_in":0,"scope":"https://www.googleapis.com/auth/gmail.readonly","token_type":"Bearer"}','{"access_token":"a","expires_in":3600,"refresh_token":"r","refresh_token_expires_in":31536001,"scope":"https://www.googleapis.com/auth/gmail.readonly","token_type":"Bearer"}','{"access_token":"a","expires_in":3600,"refresh_token":"r","refresh_token_expires_in":1.5,"scope":"https://www.googleapis.com/auth/gmail.readonly","token_type":"Bearer"}','{"access_token":"a","expires_in":3600,"refresh_token":"r","refresh_token_expires_in":"604800","scope":"https://www.googleapis.com/auth/gmail.readonly","token_type":"Bearer"}','{"access_token":"a","expires_in":3600,"refresh_token":"r","scope":"https://www.googleapis.com/auth/gmail.readonly","token_type":"Bearer","extra":true}','{"access_token":"a","expires_in":3600,"refresh_token":"r","scope":"wrong","token_type":"Bearer"}')) { try { $bad=Parse $invalid $true; $bad.Clear(); exit 64 } catch {} }; foreach($invalidRefresh in @('{"access_token":"a","expires_in":3600,"refresh_token_expires_in":0,"scope":"https://www.googleapis.com/auth/gmail.readonly","token_type":"Bearer"}','{"access_token":"a","expires_in":3600,"refresh_token_expires_in":1.5,"scope":"https://www.googleapis.com/auth/gmail.readonly","token_type":"Bearer"}')) { try { $bad=Parse $invalidRefresh $false; $bad.Clear(); exit 65 } catch {} }; exit 0`,
      ],
      {
        shell: false,
        windowsHide: true,
