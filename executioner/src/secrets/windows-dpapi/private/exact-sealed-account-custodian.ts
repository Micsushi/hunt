...
export interface ExactSealedAccountRequest {
  readonly handleId: string;
  readonly journeyId: JourneyId;
  readonly expiresAt: string;
}

export interface ExactSealedAccountCustodianOptions {
...
export class ExactSealedAccountCustodian {
  readonly #root: string;
  readonly #now: () => string;
  readonly #aclProtector: ExactRecordAclProtector;

  constructor(options: ExactSealedAccountCustodianOptions) {
  ...
  async prepare(request: ExactSealedAccountRequest): Promise<PreparedExactAccount> {
    const issuedAt = this.#now();
    if (!validRequest(request, issuedAt)) {
      throw new Error("exact account request invalid");
    }
    const destination = join(this.#root, `${request.handleId}.s2secret`);
    if (await exists(destination)) throw new Error("exact account handle already exists");
    ...
    return new PreparedExactAccount(
      this.#root,
      destination,
      metadata,
      this.#aclProtector,
...
export class PreparedExactAccount {
  readonly #root: string;
  ...
  entropy(): Uint8Array {
    if (this.#consumed) throw new Error("exact account provisioning consumed");
  ...
  async commit(
    sealed: Uint8Array,
    signal: AbortSignal,
  ): Promise<SecretHandleMetadataV1> {
    if (this.#consumed) {
      sealed.fill(0);
      throw new Error("exact account handle already exists");
    }
    try {
      if (signal.aborted) throw new Error("exact account provisioning cancelled");
      if (sealed.byteLength < 1 || sealed.byteLength > MAX_CIPHERTEXT_BYTES) {
        throw new Error("exact account ciphertext invalid");
      }
      if (await exists(this.#destination)) {
        throw new Error("exact account handle already exists");
      }
      this.#consumed = true;
      ...
      try {
        ...
        await deleteWrittenRecord(this.#root, this.#metadata.handleId);
        throw new Error("exact account store unavailable");
      }
...
async function deleteWrittenRecord(root: string, handleId: string): Promise<void> {
  await deleteSecretRecord(root, handleId);
}

function validRequest(request: ExactSealedAccountRequest, issuedAt: string): boolean {
  ...
  return HANDLE_PATTERN.test(request.handleId) &&
    ...
    expires > issued && expires - issued <= MAX_RETENTION_MS;
}
